<template>
  <!-- begin:: Aside -->
  <div
    class="aside aside-left aside-fixed d-flex flex-column flex-row-auto submenu-mobile"
    id="kt_aside"
    ref="kt_aside"
  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <KtSubMenu v-bind:menuItems="menuItems"> </KtSubMenu>
        </perfect-scrollbar>
      </div>
    </div>
  </div>
...
import KtSubMenu from "@/view/layout/aside/SubMenu.vue";
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      // Init Aside
      KTLayoutSubmenu.init(asideRef);

      asideRef.querySelectorAll("a[class='menu-link']").forEach(item => {
        item.addEventListener("click", () => {
          KTLayoutSubmenu.getOffcanvas().hide();
        });
      });
